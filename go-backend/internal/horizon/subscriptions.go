...
func (h *SubscriptionsHandler) FetchSubscriptions(userID int64) ([]SubscriptionDTO, float64, error) {
	rows, err := query.New(h.DB).ListSubscriptions(context.Background(), userID)
	if err != nil {
...
func (h *SubscriptionsHandler) CreateSubscription(w http.ResponseWriter, r *http.Request) {
	...
	if row.BudgetID.Valid {
		id := row.BudgetID.Int64
		s.BudgetID = &id
		if name, err := q.GetBudgetName(r.Context(), query.GetBudgetNameParams{ID: id, UserID: user.ID}); err == nil {
			s.BudgetName = &name
		}
	}
	if row.DeductionID.Valid {
...
func (h *SubscriptionsHandler) UpdateSubscription(w http.ResponseWriter, r *http.Request) {
	...
	if stats, err := q.GetSubscriptionTransactionStats(r.Context(), query.GetSubscriptionTransactionStatsParams{SubscriptionID: sql.NullInt64{Int64: subID, Valid: true}, UserID: user.ID}); err == nil {
...
func (h *SubscriptionsHandler) DeleteSubscription(w http.ResponseWriter, r *http.Request) {
	...
	deleted, err := query.New(h.DB).DeleteSubscription(r.Context(), query.DeleteSubscriptionParams{ID: subID, UserID: user.ID})
	if err != nil {
		webutil.ServerError(w, err)
		return
	}
...
func (h *SubscriptionsHandler) ListSubscriptionTransactions(w http.ResponseWriter, r *http.Request) {
	user, err := h.sessionUser(r)
	if errors.Is(err, sql.ErrNoRows) {
		webutil.Unauthorized(w, "session is invalid or expired")
		return
	...
	rows, err := query.New(h.DB).ListSubscriptionTransactions(r.Context(), query.ListSubscriptionTransactionsParams{UserID: user.ID, SubscriptionID: sql.NullInt64{Int64: subID, Valid: true}})
	if err != nil {
		webutil.ServerError(w, err)
		return
	}
