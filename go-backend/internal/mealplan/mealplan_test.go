...
func TestRepository(t *testing.T) {
	...
	created, err := q.CreatePlan(ctx, CreatePlanParams{UserID: 1, Column2: "2026-09-21", Name: "Salad"})
	if err != nil {
		t.Fatal(err)
	}
	...
	updated, err := q.SetPlanConsumed(ctx, SetPlanConsumedParams{IsConsumed: true, ID: created.ID, UserID: 1})
	if err != nil || !updated.IsConsumed {
		t.Fatalf("set consumed: %+v, %v", updated, err)
	}
	changed, err := q.UpdatePlan(ctx, UpdatePlanParams{Name: "Soup", Column2: "2026-09-22", IsConsumed: true, ID: created.ID, UserID: 1})
	if err != nil || changed.Name != "Soup" {
		t.Fatalf("update meal: %+v, %v", changed, err)
	}
	rows, err := q.ListPlansByDate(ctx, ListPlansByDateParams{UserID: 1, Column2: "2026-09-22"})
...
func TestHandler_Unauthorized(t *testing.T) {
	...
	t.Run("list plans without session", func(t *testing.T) {
	...
	t.Run("create plan without session", func(t *testing.T) {
	...
	t.Run("delete plan without session", func(t *testing.T) {
	...
	t.Run("update plan without session", func(t *testing.T) {
	...
	t.Run("list times without session", func(t *testing.T) {
	...
	t.Run("delete time without session", func(t *testing.T) {
		req := httptest.NewRequest(http.MethodDelete, "/api/meal-times/1", nil)
		rec := httptest.NewRecorder()
		h.DeleteTime(rec, req)
...
func TestParseTimeFlexible(t *testing.T) {
	...
	for _, tc := range tests {
		...
		if tc.expectedValid && got != tc.expectedTime {
			t.Errorf("expected %q for input %q, got %q", tc.expectedTime, tc.input, got)
		}
	}
}
...
func TestHandler_Routes(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	h.CreatePlan(rec, httptest.NewRequest(http.MethodPost, "/api/meal-plans", bytes.NewBufferString(`{"date":"2026-09-21","name":"Salad"}`)))
	...
	rec = httptest.NewRecorder()
	h.UpdatePlanConsumed(rec, requestWithID(http.MethodPatch, "/api/meal-plans/1/consumed", `{"is_consumed":true}`, meal.ID))
	if rec.Code != http.StatusOK {
		t.Fatalf("consume: %d %s", rec.Code, rec.Body.String())
	}
	rec = httptest.NewRecorder()
	h.ListPlans(rec, httptest.NewRequest(http.MethodGet, "/api/meal-plans?date=2026-09-21", nil))
	if rec.Code != http.StatusOK {
